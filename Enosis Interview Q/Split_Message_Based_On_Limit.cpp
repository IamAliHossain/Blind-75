/*
    ...
    Problem Name : 2468. Split Message Based on Limit
    Difficulty : Hard
...
===========================================================================================
    ...
    suffix "<a/b>", where "b" is to be replaced with the total number of parts and "a" is to be 
    replaced with the index of the part, starting from 1 and going up to b. Additionally, the length 
    of each resulting part (including its suffix) should be equal to limit, except for the last part 
...
public:
    ...
    vector<string> SplitMessageBasedOnLimit(string &message, int limit, int digits){
        vector<string> result;
        
        int actual_length = message.size();
        int a = 1;
        string b = "";
        for(int i=0; i<digits; i++) b+="-";
        int j = 0;
        while(j < actual_length){
            string suffix = "<" + to_string(a) + "/" + b + ">";
            a++;

            int charRemain = limit - suffix.length();
            if(charRemain <= 0) return {};

            string prefix = "";
            while(j < actual_length && charRemain > 0){
                prefix += message[j];
                j++;
                charRemain--;
            }
        ...
        int total_parts = result.size();
        int digits_cnt_of_total_parts = cntDigit(total_parts);
        if(digits_cnt_of_total_parts != digits){
            return {};
        }

        a = 1;
        for(auto &s : result){
            string suffix = "<" + to_string(a) + "/" + to_string(total_parts) + ">";
            s += suffix;
            a++;
        }
        return result;
    ...
    vector<string> splitMessage(string message, int limit) {
        ...
        for(int digits = 1; digits <5; digits++){
            vector<string> ans = SplitMessageBasedOnLimit(message, limit, digits);
