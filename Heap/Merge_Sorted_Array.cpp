/*
    ...
    This problem is form Coding LeetCode Platform, listed on 450 DSA cracker(From Heap)
...
public:
    void merge(vector<int>& nums1, int m, vector<int>& nums2, int n) {
        ...
        nums1.clear();
        while(!minHeap.empty()){ // O(m + n)
            nums1.push_back(minHeap.top()); // O(1)
            minHeap.pop(); // O(long(m+n))
        }
