...
public:
    void dfs(int i, int j, int n, int m, vector<vector<int>>& heights, vector<vector<bool>> & visited){
        ...
        visited[i][j] = true;
        if(i+1 <n && heights[i+1][j] >= heights[i][j] && !visited[i+1][j]){
            dfs(i+1, j, n, m, heights, visited);
        }
        if(i-1 >= 0 && heights[i-1][j] >= heights[i][j] and !visited[i-1][j]){
            dfs(i-1, j, n, m, heights, visited);
        }
        if(j+1 < m && heights[i][j+1] >= heights[i][j] and !visited[i][j+1]){
            dfs(i, j+1, n, m, heights, visited);
        }
        if(j-1 >= 0 && heights[i][j-1] >= heights[i][j] and !visited[i][j-1]){
            dfs(i, j-1, n, m, heights, visited);
        }
